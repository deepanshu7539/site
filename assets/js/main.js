...
const contactForm = document.getElementById('contact-form'),
      ...
      contactMesssge = document.getElementById('contact-message')
...
const sendEmail = (e) => {
    ...
    if(contactName.value === '' || contactEmail.value === '' || contactProject.value === ''){
        ...
        contactMesssge.classList.remove('color-blue')
        contactMesssge.classList.add('color-red')

        //show message
        contactMesssge.textContent = 'Write all the input fields 📩'
    }else{
        //serviceId - templateID - #from - publicKey
        emailjs.sendForm('service_s9oyt1q','template_k6msty5','#contact-form','HKA5aRo3jjS3R_JAd')
         .then(() =>{
            //show message and add color
            contactMesssge.classList.add('color-blue')
            contactMesssge.textContent = 'Message sent ✅'

            //Remove message after five second
            setTimeout(() =>{
                contactMesssge.textContent = ''
            }, 5000)
         }, (error) =>{
            alert('OOPS! SOMETHING HAS FAILED...', error)
         })

         //To clear the input files
         contactName.value = ''
         contactEmail.value = ''
